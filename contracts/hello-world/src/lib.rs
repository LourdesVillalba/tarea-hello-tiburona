...
mod test {
    ...
    #[test]
    #[should_panic(expected = "Error(Contract, #4)")] // Era #4 = NoInicializado
    ...
    #[test]
    #[should_panic(expected = "Error(Contract, #1)")] // Era #1 = NombreVacio
    ...
    #[test]
    #[should_panic(expected = "Error(Contract, #3)")] // Era #3 = NoAutorizado
    fn test_reset_no_autorizado() {
        ...
        let admin = Address::generate(&env);
        let _otro = Address::generate(&env); // Agregado _ para evitar warning
        
        client.initialize(&admin);
        
        client.reset_contador(&_otro);
    }
}